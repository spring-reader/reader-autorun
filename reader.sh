...
trap 'kill $BACKUP_PID; kill $JAVA_PID' EXIT SIGKILL
...
reader_jar_get_latest() {
	...
	[[ $? != 0 ]] && (rm -f $new_file_name; exit 222)

	[[ "${file_name}" != "reader*.jar" ]] &&  rm -rf ${file_name}*
}

reader_jar_get_static() {
	local file_name=$(basename reader*.jar)
	[[ "${file_name}" =~ "${READER_JAR_STATIC_VERSION}" ]] && return
...
reader_jar_download() {
	...
	[[ -e $(basename reader*.jar) ]] || exit 255
}
